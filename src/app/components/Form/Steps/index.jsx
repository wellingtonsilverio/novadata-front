...
async function FormSteps({step, nextStep, backStep}) {
    ...
    <div className={styles.Steps}>
        ...
        {formData.footer && <div className={styles.footer}>{formData.footer?.map(footerItem => {
          if (footerItem?.buttons) {
            return <div key={footerItem.id} className={styles.buttons}>{footerItem?.buttons?.map(button => <button className={[styles[button.button], styles.button].join(' ')} key={button.id} onClick={button.button === 'next' ? nextStep : button.button === 'back' ? backStep : undefined}>{getButtonLabel(button.button)}</button>)}</div>
          }
        })}</div>}
      </div>
  );
